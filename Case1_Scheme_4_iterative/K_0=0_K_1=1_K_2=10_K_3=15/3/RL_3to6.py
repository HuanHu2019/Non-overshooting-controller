...
gaocha_1 = 0.0023581222923136913

gaocha_2 = 0.00016244941798454482

gaocha_3 = 1.2198005950536661e-05
...
# mingzi ='pid_'+ str(int(initial * 10)) +'to'+ str(int(end * 10))+'.npy'

# data = np.load(mingzi)

# fuckit = data [:,[0,2,3,4,5,6,7,10,11]]
...
# acflapmax = max(acit_flap)

# if acflapmax+flapjian > flap_max:
    
#     acflapshang = flap_max
    
# else:
    
#     acflapshang = acflapmax+flapjian 
    
    
    
# acflapmin = min(acit_flap)

# if acflapmin-flapjian < -flap_max:
    
#     acflapxia = -flap_max
    
# else:
    
#     acflapxia = acflapmin - flapjian
...
# acTmax = max(acit_Thrust)

# if acTmax + Tjian > Tzhong:
    
#     acTshang = Tzhong
    
# else:
    
#     acTshang = acTmax + Tjian 
    
    
    
# acTmin = min(acit_Thrust)
...
# acit_data = np.array([[acflapshang,acTshang],[acflapxia,acTxia]])
...
class Fuckingfly:
    ...
    def reset(self,casenum=1):
        ...
        # state_0_yuan_2D = state_0_yuan.reshape(1,-1)
        
        # state_0_temp = guiyi.transform(state_0_yuan_2D)
        
        # state_0 = state_0_temp[0]
    ...
    def step(self, action):
        ...
        shineng_now,reward_fir,reward_sec,reward_third,reward_forth = self.shinengjisuan(self.initial,    self.end ,   fuck_fu[-1])
        
        #reward = (shineng_now *self.gamma - self.shineng_last) + reward_bu
        # print('reward_fir',reward_fir)

        # print('reward_sec',reward_sec)
        
        # print('reward_third',reward_third)

        reward = shineng_now + reward_fir + reward_sec + reward_third + reward_forth
       ...
       # print('reward',reward)
        ...
        ######## 状态归一化
        # state_next_yuan_2D = state_next_yuan.reshape(1,-1)
        
        # state_next_temp = guiyi.transform(state_next_yuan_2D)
        
        # state_next = state_next_temp[0]
    ...
    def quanrender(self,casenum=1):
        ...
        quanhuatu(self.jilustates, self.Tzhong, self.flap_max, self.initial, self.end, self.hua_x, self.shijianbu)
        
    def render(self,casenum=1):
        
        huatu(self.jilustates, self.Tzhong, self.flap_max, self.initial, self.end, self.hua_x, self.shijianbu)
    ...
    def shinengjisuan(self,initial,end,feigao):
        ...
        if self.jiang1 == 1:
            ...
            zhongzhuan,yueguoma = suanzi(initial,end,overshoot,feigao)
            
            if zhongzhuan == 0 and yueguoma:
                    
                self.jiang1 = 0
                
                reward_1 = 0
            
            if yueguoma is False:
                     
                reward_1 =  r_1 * zhongzhuan
                
        


        # overshoot_2 = gaocha_2 
        ...
        #     zhongzhuan_2 = suanzi(initial,end,overshoot_2,feigao)
        ...
        #     zhongzhuan = suanzi(initial,end,overshoot,feigao)
        ...
        if gao_n == 0:
             ...
             return cost_n, 0, 0, 0, 0


        if gao_n == 1:

             return cost_n, reward_1, 0, 0, 0

        if gao_n == 2:

             return cost_n, reward_1, reward_2, 0, 0


        if gao_n == 3:

             return cost_n, reward_1, reward_2, reward_3, 0
            
    
        if gao_n == 4:

             return cost_n, reward_1, reward_2, reward_3, reward_4    
    
    
    

# def suanzi(initial,end,overshoot,feigao):
...
#     out = fazhi *(1-np.clip(juli/overshoot,0,1))
    
#     if initial > end:
        
#         if feigao > end + overshoot*(1.0*(fazhi-1)/fazhi):
...
def suanzi(initial,end,overshoot,feigao):
    ...
    out = (1-np.clip(juli/overshoot,0,1))
    
    if initial < end:
        
        if feigao > end + overshoot:
...
#         # jieshou_1 = gaocha_1 #  0.4*0.01#       *0.3*0.02
        
#         # # cost_n_jieshou = 1 - np.clip(jieshou/fenmu,0,1) 
        
#         # # xishu = zhong/cost_n_jieshou
...
#         #     reward_pan = 1- np.clip(juli/jieshou_1,0,1) 
        

#         # jieshou_2 =  gaocha_2 #  0.4*0.01#       *0.3*0.02
...
#             reward_pan =  10000* np.log( zhongzhuan )
        
#         #print('reward_pan',reward_pan)
...
#         if feigao_aim - jieshou_2 <= feigao <= feigao_aim + jieshou_2:
            
#             reward_pan_2 = 1- np.clip(juli/jieshou_2,0,1)      

    

#         jieshou_3 = gaocha_3 #  0.4*0.01#       *0.3*0.02
...
#         if feigao_aim - jieshou_3 <= feigao <= feigao_aim + jieshou_3:
            
#             reward_pan_3 = 1- np.clip(juli/jieshou_3,0,1) 
            
            
            
            
#         jieshou_4 = gaocha_4 #  0.4*0.01#       *0.3*0.02
...
#         if feigao_aim - jieshou_4 <= feigao <= feigao_aim + jieshou_4:
            
#             reward_pan_4 = 1- np.clip(juli/jieshou_4,0,1) 
...
#                 #return cost_n*1,1,1,reward_pan*40,reward_pan_2*1600,reward_pan_3*40*40*40  #reward_pan*20
#                     return cost_n*1,1,1,1,1,1,1
                    
#                 if gao_n == 1:
        
#                 #return cost_n*1,1,1,reward_pan*40,reward_pan_2*1600,reward_pan_3*40*40*40  #reward_pan*20
#                     return cost_n*1,1,1,reward_pan,1,1,1
                
#                 if gao_n == 2:
        
#                 #return cost_n*1,1,1,reward_pan*40,reward_pan_2*1600,reward_pan_3*40*40*40  #reward_pan*20
#                     return cost_n*1,1,1,reward_pan*(jieshoubu+2),reward_pan_2*(jieshoubu+2)*(jieshoubu+2),0,0
                
#                 if gao_n == 3:
        
#                 #return cost_n*1,1,1,reward_pan*40,reward_pan_2*1600,reward_pan_3*40*40*40  #reward_pan*20
#                     return cost_n*1,1,1,reward_pan*(jieshoubu+2),reward_pan_2*(jieshoubu+2)*(jieshoubu+2),reward_pan_3*(jieshoubu+2)*(jieshoubu+2)*(jieshoubu+2),0
        
#                 if gao_n == 4:
        
#                 #return cost_n*1,1,1,reward_pan*40,reward_pan_2*1600,reward_pan_3*40*40*40  #reward_pan*20
#                     return cost_n*1,1,1,reward_pan*(jieshoubu+2),reward_pan_2*(jieshoubu+2)*(jieshoubu+2),reward_pan_3*(jieshoubu+2)*(jieshoubu+2)*(jieshoubu+2),  reward_pan_4*(jieshoubu+2)*(jieshoubu+2)*(jieshoubu+2)*(jieshoubu+2)
...
#                 #return cost_n*1,1,1,reward_pan*40,reward_pan_2*1600,reward_pan_3*40*40*40  #reward_pan*20
#                     return cost_n*1,1,1,reward_pan,0,0,0           
                
#                 if gao_n == 2:
        
#                 #return cost_n*1,1,1,reward_pan*40,reward_pan_2*1600,reward_pan_3*40*40*40  #reward_pan*20
#                     return cost_n*1,1,1,reward_pan,reward_pan_2,0,0
                
#                 if gao_n == 3:
        
#                 #return cost_n*1,1,1,reward_pan*40,reward_pan_2*1600,reward_pan_3*40*40*40  #reward_pan*20
#                     return cost_n*1,1,1,reward_pan,reward_pan_2,reward_pan_3,0
        
#                 if gao_n == 4:
        
#                 #return cost_n*1,1,1,reward_pan*40,reward_pan_2*1600,reward_pan_3*40*40*40  #reward_pan*20
#                     return cost_n*1,1,1,reward_pan,reward_pan_2,reward_pan_3,reward_pan_4           
...
def huatu(jilustates,  Tzhong,    flap_max,    initial,    end,    hua_x,   shijianbu):
    ...
    mingcheng = pathh+'\\'+ wenjianjia+ '\\' + str(fucknum)+'.png'
    
    #data_save = os.path.join(pathh, '\\', wenjianjia, '\\', str(fucknum)+'.npy')
    data_save =pathh+ '\\'+ wenjianjia+ '\\'+ str(fucknum)+'.npy'
    ...
    axes1.set_ylim([(-0.31*0.25+ 0.3)*0.4, (0.31*0.25+ 0.6)*0.4])
    axes1.set_xlim([0, hua_x])
...
def quanhuatu(jilustates,  Tzhong,    flap_max,    initial,    end,    hua_x,   shijianbu):
